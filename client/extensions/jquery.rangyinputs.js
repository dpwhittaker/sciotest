...
(function($) {
    ...
    $(document).ready(function() {
        ...
        selectLines = function(el) {
            var sel = getSelection(el), val = el.value;
            var start = val.lastIndexOf("\n", sel.start - 1) + 1;
            var end = val.indexOf("\n", start == sel.end ? sel.end : sel.end - 1);
            end = end == -1 || end == val.length ? end = val.length : end + 1;
            setSelection(el, start, end);
        };
        
        prependSelectedLines = function(el, text, selectionBehavior) {
            var initialSel = getSelection(el);
            selectLines(el);
            var sel = getSelection(el);
            sel.text = text + sel.text.replace(/\n(?!$)/g, "\n" + text);
            replaceSelectedText(el, sel.text, selectionBehavior || 'select');
            if (initialSel.start == initialSel.end) setSelection(el, initialSel.start + text.length, initialSel.end + text.length);
        };
        ...
        $.fn.extend({
            ...
            selectLines: jQuerify(selectLines, true),
            prependSelectedLines: jQuerify(prependSelectedLines, true)
        });
    });
})(jQuery);